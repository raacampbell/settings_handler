...
% Rob Campbell - Basel 2015
	...
	methods
		function obj=settings_handler(settingsFname)
			...
			%read the yml file
			Y=settings.yaml.ReadYaml(settingsFname);

			if ~exist(Y.default,'file')
				error('Can not find settings file %s\n', Y.default)
			end
			obj.files.defaultFile = Y.default;
			obj.defaultSettings = settings.yaml.ReadYaml(obj.files.defaultFile);
			...
			if ~exist(obj.files.userFile)
				...
				settings.yaml.WriteYaml(obj.files.userFile, obj.defaultSettings);
			end
		...
		function out = subsref(obj,S)
	 	...
	 	function out=subsasgn(obj,S,newValue)
			...
			if ~isa(thisObject,'setting')
		 	...
		 	userSettings = settings.yaml.ReadYaml(obj.files.userFile);

		 	userSettings = subsasgn(userSettings,S,newValue);

		 	settings.yaml.WriteYaml(obj.files.userFile,userSettings);
